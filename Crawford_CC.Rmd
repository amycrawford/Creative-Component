...
```{r, message=F, warning=F, fig.height=.5, echo = F, fig.align="center", fig.width = 7}
ggplot() + ggtitle("Table 6", subtitle = "Confusion Matrix and Statistics for Classifaction by Support Vector Machine (SVM) with 13 Predictor Variables                                           ") + theme_minimal()
```
\begin{center}
\footnotesize
\begin{tabular}{l r c c}
...
```{r, message=F, warning=F, fig.height=.5, echo = F, fig.align="center", fig.width = 7}
ggplot() + ggtitle("Table 7", subtitle = "Confusion Matrix and Statistics for Classifaction by Support Vector Machine (SVM) with 15 Predictor Variables                                          ") + theme_minimal()
```
\begin{center}
\footnotesize
\begin{tabular}{l r c c}
...
```{r, message=F, warning=F, fig.height=.5, echo = F, fig.align="center", fig.width = 7}
ggplot() + ggtitle("Table 8", subtitle = "Confusion Matrix and Statistics for Classifaction by Random Forest 13 Predictor Variables                                           ") + theme_minimal()
```
\begin{center}
\footnotesize
\begin{tabular}{l r c c}
...
```{r, message=F, warning=F, fig.height=.5, echo = F, fig.align="center", fig.width = 7}
ggplot() + ggtitle("Table 9", subtitle = "Confusion Matrix and Statistics for Classifaction by Random Forest 15 Predictor Variables                                           ") + theme_minimal()
```
\begin{center}
\footnotesize
\begin{tabular}{l r c c}
...
> where $\lambda$ can be thought of as a tuning parameter, and $d_k(u,x)$ denotes a distance (or dissimilarity) function of $u$ and $x$ on the $k^{th}$ coordinate of the feature vector. For standardized numeric features use Euclidean distance $$d_k(u,x) = \sqrt{(u_{jk} - x_{ik})^2}.$$ 
   
> For all other categorical features use the following weighted degree-of-difference measure to assess dissimilarity for these features [@hastie_tibshirani_friedman]
...
\end{array} 
\right., \]
where $\tau$ is the weight we will give to a pair of dissimilar categorical features at every coordinate of a categorical variables $k$.
...
$\quad$ Because a subset of the testing cases were used in runs of this algorithm, the reference distribution of `functional` and `requires attention` wells will likely differ from the overall distribution for the entire set $\mathcal{P}$ (i.e. the no information rates will differ for subsets of $\mathcal{P}$). , While the Accuracy values are certainly informative, they may be a bit misleading. Because the Kappa statistic takes into account the accuracy that would be generated simply by chance [@kuhn_johnson] it can be used to compare ASSC-SVM models to eachother as well as to all other models presented here.
...
\begin{tabular}{l l c c c c}
    ...
    \multirow{ 2}{*}{SVM}& 13 Predictors & 0.6754 & 0.3533 & 0.8192 & 0.5785 \\ 
    & 15 Predictors & 0.6759 & 0.3535 & 0.8192 & 0.5909 \\ \hline
    \multirow{ 2}{*}{Random Forest}& 13 Predictors & 0.6711 & 0.3455 & 0.8432 & 0.6414 \\ 
    & 15 Predictors & 0.6790 & 0.3568 & 0.8433 & 0.6384 \\ \hline
    \multirow{ 2}{*}{ASSC-SVM}& 13 Predictors & ?0.7(1.5more) & ?0.4(1.5more) & - & - \\ 
    & 15 Predictors & 0.7233(1more) & 0.4424(1more) & - & - \\ \hline
\end{tabular}
\end{center}
