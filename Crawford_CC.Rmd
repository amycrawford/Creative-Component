...
shape <- readShapeSpatial("/Users/amy/Documents/CC/Creative Component/TZA_adm_shp/TZA_adm1.shp")
...
map2 <- country + geom_point(data = data[data$latitude != 0 & data$longitude != 0,],
                     aes(x = longitude, y = latitude, colour = status_group), size = 0.5) +
  ggtitle("", subtitle = "\nExclude Water Points with \n'Zero' Surrounding Population") +
...
train_logistic_regression$amount_tsh <- (train_logistic_regression$amount_tsh - mean(train_logistic_regression$amount_tsh))/sd(train_logistic_regression$amount_tsh)
train_logistic_regression$gps_height <- (train_logistic_regression$gps_height - mean(train_logistic_regression$gps_height))/sd(train_logistic_regression$gps_height)

# some (rare) factors exist in the training set but not the testing set, rename them.
# train_logistic_regression[train$extraction_type_class %in% "rope pump",]$extraction_type_class <- "other"
# train_logistic_regression[train$waterpoint_type %in% "dam",]$waterpoint_type <- "other"
...
test_logistic_regression$amount_tsh <- (test_logistic_regression$amount_tsh - mean(test_logistic_regression$amount_tsh))/sd(test_logistic_regression$amount_tsh)
test_logistic_regression$gps_height <- (test_logistic_regression$gps_height - mean(test_logistic_regression$gps_height))/sd(test_logistic_regression$gps_height)
...
# a <- anova(model_logistic_regression, test = "Chisq")
# 
...
```{r, message=F, warning=F, fig.height=.5, echo = F, fig.align="center", fig.width = 7}
library(ggplot2)
ggplot() + ggtitle("Table 4", subtitle = "Confusion Matrix and Statistics for Classifaction by Logistic Regression                                           ") + theme_minimal()
```
...
# #train_small <- train_logistic_regression_reorder[sample(nrow(train_logistic_regression_reorder), 500),]
# test_small <- test_logistic_regression_target[sample(nrow(test_logistic_regression_target), 300),]
# 
...
# tau <- 1  # set weight for categorical variable distance peice of function
# lambda <- 3
# 
# d <- function(u, x){
...
#     w[j,i] <- exp(-d(subset(test_small[j,], select = -c(test_y)), subset(train_small, select = -c(status_group))[i,])/lambda)
#   }
# } 
# 
# ### STEP 2
...
#   for(j in 1:nrow(test_small)){  
#     
#     q <- 180  #size of weighted bootstrap sample
#     active_set_b <- c()
#     for(l in 1:q){
...
#       predictions[j, b] <- ifelse(predict(object = model_svm_b, newdata = subset(test_small[j,], select = -c(test_y))) == "functional", "functional", "requires attention")
#     }
#   }
# } 
# 
