...
## Abstract  
When companies implement algorithms to expedite existing job functions it often leaves them with newly available employees, time, and money to investigate other unexplored parts of their market. To be as efficient as possible in an expansion such as this, the company can again turn to some machine learning algorithm to point to where the newly available resources should be focused in the market. Unfortunately, the only available data to train an algorithm on would then be data from the portion of the market that has already been explored by the company. This leads to many scenarios in which a case in the data set will either have a label or not have one due to a specific non-random mechanism. In this paper, I evaluate the performance of traditional methods of classification such as logistic regression, support vector machine (SVM), and random forest models under this framework. Additionally, I implement one classification method called active set selection classification (ASSC) that involves weighted bootstrap and emsemble learning.
...
$\quad$ There are variables in the data set that appear to be manually maintained and have too many unique values to be managable. These variables were eliminated as well as any variables with only one unique value. All variables providing information on the location of the water point were eliminated with the exception of `basin`. A few other variables were excluded for lack of predictive power. The variable `basin` was kept because it gives information on the geographic location of the water points with regard to the water basin they pull from. The data began with 40 feature columns (excluding the target variable, `status_group`). After feature selection, the resulting data set has 15 feature columns and the target variable. Since the `population` and `construction_year` variables were used to create the split between training and testing data sets I will also fit models with and without these variables to compare performance. Table 3 is a compilation of the final 15 features, their descriptions, and values that each can take.
...
\begin{tabular}{l l p{7cm}}
  ...
  \textit{population*} & Numeric, (1, 30500) & Population around the well\\
  \textit{construction\_year*} & Numeric, (1960, 2013) & Year the waterpoint was constructed\\
...
\footnotesize
\begin{tabular}{l}
  \textit{*Variables used to create split between training and testing sets.                                                                                                                                     }\\
  \end{tabular}
...
## Analysis
Four prediction methods are detailed in the following portion of the paper. Each model was built on the "cornered" training data set excluding the `population` and `construction_year` variables (13 predictors) and then again with those two variables included (15 predictors). 

\vspace{5mm}

### Logistic Regression
$\quad$ Logistic regression has a long history in the field of statistics and is one of the most widely used source of classifiers [@james_witten_hastie_tibshirani]. In general, it is a method of estimating conditional probabilities as a function of explanatory variables and regression coefficients, and is often used to classify binary response data which is how we will apply it here. Two logistic regression models were fit to the training data using the `glm` function in `R`. For logistic regression, categorical variables (stored as `factors` in `R`) are expanded into binary indicator variables. The full equations for the 13 predictor and 15 predictor models can both be found in the appendix.   

$\quad$ Using the 13-predictor and 15-predictor models to classify water pumps in the testing set yields the confusion matrices and statistics in Tables 4 and 5 respectively. Since we are extrapolating, the relatively low values of Accuracy, Sensitivity, and Specificity are not surprising. Kuhn and Johnson (2013) write that Kappa values within 0.30 to 0.50 indicate reasonable agreement, so although they are on the low end, we are not so dissatisfied with the Kappa statistics of 0.3703 and 0.3748. 10-Fold cross validation results are reported in Table 10.
...
preds_logistic_regression_15 <- predict(model_logistic_regression_15_CV, newdata = test_15)
preds_logistic_regression_13 <- predict(model_logistic_regression_13_CV, newdata = test_13)

cm_logistic_regression_15 <- confusionMatrix(data=preds_logistic_regression_15, test_y)
cm_logistic_regression_13 <- confusionMatrix(data=preds_logistic_regression_13, test_y)
...
```{r, message=F, warning=F, fig.height=.5, echo = F, fig.align="center", fig.width = 7}
ggplot() + ggtitle("Table 4", subtitle = "Confusion Matrix and Statistics for Classification by Logistic Regression with 13 Predictor Variables                                          ") + theme_minimal()
```
...
\begin{tabular}{l r c c}
    ...
    Prediction & \multicolumn{1}{r|}{functional} & 13138 & 4206 \\
    & \multicolumn{1}{r|}{requires attention} & 6632 & 10482 \\
\end{tabular}
\begin{tabular}{llrl}
    &&& \\
    &    &   Kappa :&0.3703\\
...
```{r, message=F, warning=F, fig.height=.5, echo = F, fig.align="center", fig.width = 7}
ggplot() + ggtitle("Table 5", subtitle = "Confusion Matrix and Statistics for Classification by Logistic Regression with 15 Predictor Variables                                           ") + theme_minimal()
```
...
\begin{tabular}{l r c c}
    & & \multicolumn{2}{ c }{Reference} \\
    & & functional & requires attention \\ \cline{3-4} 
    Prediction & \multicolumn{1}{r|}{functional} & 13302 & 4271 \\
    & \multicolumn{1}{r|}{requires attention} & 6468 & 10417 \\
\end{tabular}
\begin{tabular}{llrl}
    &&& \\
    &    &   Kappa : &0.3748\\
...
#tune cost parameter
#svm_tune_13 <- tune.svm(status_group ~ . , data = train_13, tuneControl = tune.control(nrepeat = 3, cross = 10), cost = c(20, 25, 30, 35, 40, 45, 50))
#svm_tune_15 <- tune.svm(status_group ~ . , data = train_13, tuneControl = tune.control(nrepeat = 8, cross = 10), cost = c(45, 50, 60, 70, 80, 90))

## 3-time, 10-fold CV
svmCV <- function(train_set, cost) {
  out <- data.frame(accs = c(NA, NA, NA), kaps = c(NA, NA, NA))
  for(j in 1:3){
    folds <- createFolds(train_set$status_group, k = 10, list = TRUE, returnTrain = TRUE)
    accuracies <- kappas <- c()
    for (i in 1:10) {
        model <- svm(status_group~., data = train_set[folds[[i]],], kernel = "radial", cost = cost)
        predictions <- predict(object = model, newdata = subset(train_set[-folds[[i]],], select = -c(status_group)))
        cm <- confusionMatrix(predictions, train[-folds[[i]], ]$status_group)
        accuracies <- c(accuracies, cm$overall[[1]])
        kappas <- c(kappas, cm$overall[[2]])
    }
    out[j,] <- c(mean(accuracies), mean(kappas))
  }
  c(Accuracy = mean(out$accs), Kappa = mean(out$kaps))
}  

# get CV accuracy and kappa
stats_svm_13 <- svmCV(train_13, cost = 40)
stats_svm_15 <- svmCV(train_15, cost = 90)

#Train models
svm_13 <- svm(status_group ~ . , data = train_13, cost = 40)
svm_15 <- svm(status_group ~ . , data = train_13, cost = 90)

#Make predictions & clacuk
cm_svm_13 <- confusionMatrix(predict(svm_13, newdata = test_13), test_y)
cm_svm_15 <- confusionMatrix(predict(svm_15, newdata = test_15), test_y)
```



```{r, message=F, warning=F, fig.height=.5, echo = F, fig.align="center", fig.width = 7}
ggplot() + ggtitle("Table 6", subtitle = "Confusion Matrix and Statistics for Classification by Support Vector Machine (SVM) with 13 Predictor Variables                                           ") + theme_minimal()
```
\begin{center}
\footnotesize
\begin{tabular}{l r c c}
    & & \multicolumn{2}{ c }{Reference} \\
    & & functional & requires attention \\ \cline{3-4} 
    Prediction & \multicolumn{1}{r|}{functional} & 12683 & 4097 \\
    & \multicolumn{1}{r|}{requires attention} & 7087 & 10591 \\
\end{tabular}
\begin{tabular}{llrl}
    &&& \\
    &    &  Kappa: &0.3533\\
...
```{r, message=F, warning=F, fig.height=.5, echo = F, fig.align="center", fig.width = 7}
ggplot() + ggtitle("Table 7", subtitle = "Confusion Matrix and Statistics for Classification by Support Vector Machine (SVM) with 15 Predictor Variables                                          ") + theme_minimal()
```
\begin{center}
\footnotesize
\begin{tabular}{l r c c}
    & & \multicolumn{2}{ c }{Reference} \\
    & & functional & requires attention \\ \cline{3-4} 
    Prediction & \multicolumn{1}{r|}{functional} & 12764 & 4161 \\
    & \multicolumn{1}{r|}{requires attention} & 7006 & 10527 \\
\end{tabular}
\begin{tabular}{llrl}
    &&& \\
    &    &  Kappa: &0.3535\\
...
### Random Forest
$\quad$ Random forest is a tree-based method that can be used for classification. This algorithm builds a number of decision trees on bootstrapped training samples. During building, at each node (or split) of each tree, the algorithm radomly selects a subset of input features and finds an optimal single split. For each tree, we repeat splitting in this fashion up to a fixed depth or until no single split improvement is possible without creating a split resulting in a small number of cases. Many trees are built in this fashion then aggregated [@james_witten_hastie_tibshirani].

$\quad$ Two random forest models were trained using the `randomForest` function from the `randomForest` package in `R`. The parameter `mtry` determines the number of features randomly sampled as candidates at each split. Again using 10-fold cross validation, the model fit with 13 predictor variables was optimized at an `mtry` value of 3 and the model fit with 15 predictor variables at 4. The resulting confusion matrices and statistics for these two models are in Tables 8 and 9. 10-Fold cross validation results are in Table 10.
...
set.seed(4)
#rf_tune <- tuneRF(x = subset(train_13, select = c(-status_group)), y = train_13$status_group, ntreeTry = 500)

## 3-time, 10-fold CV
rfCV <- function(train_set, mtry) {
  out <- data.frame(accs = c(NA, NA, NA), kaps = c(NA, NA, NA))
  for(j in 1:3){
    folds <- createFolds(train_set$status_group, k = 10, list = TRUE, returnTrain = TRUE)
    accuracies <- kappas <- c()
    for (i in 1:10) {
        model <- randomForest(status_group ~ ., data = train_set[folds[[i]],], importance = T, ntree = 1500, mtry = mtry)
        predictions <- predict(object = model, newdata = subset(train_set[-folds[[i]],], select = -c(status_group)))
        cm <- confusionMatrix(predictions, train[-folds[[i]], ]$status_group)
        accuracies <- c(accuracies, cm$overall[[1]])
        kappas <- c(kappas, cm$overall[[2]])
...
# CV in caret to choose mtry
control <- trainControl(method="repeatedcv", number=10, repeats=3, search="grid")
tunegrid <- expand.grid(.mtry=c(1:15))
rf_gridsearch <- train(status_group~., data=train_13, method="rf", metric="Kappa", tuneGrid=tunegrid, trControl=control)
rf_gridsearch_15 <- train(status_group~., data=train_15, method="rf", metric="Kappa", tuneGrid=tunegrid, trControl=control)
# Use CVed random forest to predict the values in test
preds_forest_13_CV <- predict(rf_gridsearch, test_13)
preds_forest_15_CV <- predict(rf_gridsearch_15, test_15)
# CMs for CV models
confusionMatrix(preds_forest_13_CV, test_y)
confusionMatrix(preds_forest_15_CV, test_y)

# build models
model_forest_13 <- randomForest(status_group ~ ., data = train_13, importance = T, ntree = 1500, mtry = 3)
model_forest_15 <- randomForest(status_group ~ ., data = train_15, importance = T, ntree = 1500, mtry = 4)
# Use random forest to predict the values in test
preds_forest_13 <- predict(model_forest_13, test_13)
preds_forest_15 <- predict(model_forest_15, test_15)
# CMs for randomForest models
confusionMatrix(preds_forest_13, test_y)
confusionMatrix(preds_forest_15, test_y)
# ACC and Kappa for randomForest
stats_rf_13_3 <- rfCV(train_13, 3) # ac = 0.6682, k = 0.3413
stats_rf_15_4 <- rfCV(train_15, 4) # Kappa: 0.355 , Accuracy :0.678
```


```{r, message=F, warning=F, fig.height=.5, echo = F, fig.align="center", fig.width = 7}
ggplot() + ggtitle("Table 8", subtitle = "Confusion Matrix and Statistics for Classification by Random Forest 13 Predictor Variables                                           ") + theme_minimal()
```
\begin{center}
\footnotesize
\begin{tabular}{l r c c}
    & & \multicolumn{2}{ c }{Reference} \\
    & & functional & requires attention \\ \cline{3-4} 
    Prediction & \multicolumn{1}{r|}{functional} & 12533 & 4097 \\
    & \multicolumn{1}{r|}{requires attention} &  7237 & 10591 \\
\end{tabular}
\begin{tabular}{llrl}
    &&& \\
    &    &   Kappa: &0.3455 \\
...
```{r, message=F, warning=F, fig.height=.5, echo = F, fig.align="center", fig.width = 7}
ggplot() + ggtitle("Table 9", subtitle = "Confusion Matrix and Statistics for Classification by Random Forest 15 Predictor Variables                                           ") + theme_minimal()
```
\begin{center}
\footnotesize
\begin{tabular}{l r c c}
    & & \multicolumn{2}{ c }{Reference} \\
    & & functional & requires attention \\ \cline{3-4} 
    Prediction & \multicolumn{1}{r|}{functional} & 13083 & 4373 \\
    & \multicolumn{1}{r|}{requires attention} & 6687 & 10315 \\
\end{tabular}
\begin{tabular}{llrl}
    &&& \\
    &    &   Kappa: &0.3568\\
...
> For the $k_n$ standardized numeric features use Euclidean distance $$d_k(u,x) = \sqrt{(u_{jk} - x_{ik})^2}.$$ 
   
> For the remaining $k_c$ other categorical features use the following weighted degree-of-difference measure to assess dissimilarity for these features [@hastie_tibshirani_friedman]
\[ d_k(u,x) = \tau \mbox{ I}[u_{jk} \ne x_{ik}] = \tau \left\{
\begin{array}{ll}
      1 & u_{jk} \ne x_{ik} \\
      0 & u_{jk} = x_{ik} \\
\end{array} 
...
$$  
w_{ji}=\mbox{exp}\left \{ - \ddfrac{\sum_{k=1}^{k_n}(u_{jk} - x_{ik})^2 + \tau^2\sum_{k=k_n+1}^{k_c}\mbox{ I}[u_{jk} \ne x_{ik}]}{\lambda}   \right \}.
$$
...
$\quad$ For computational convenience we implement this algorithm using a random sample of size $m = 300$ from $\mathcal{P}$ with 13 predictor variables using parameters $B = 201$, $q = 150$, $\tau = 1.8$, $\lambda = 4$. This process is repeated three times and the averaged Accuracy and Kappa statistics can be found in Table 10. Then, we implement this algorithm using a random sample of size $m = 300$ from $\mathcal{P}$ with 15 predictor variables using parameters $B = 201$, $q = 150$, $\tau = 1.5$, $\lambda = 3$. This process is also repeated three times and the averaged Accuracy and Kappa statistics are summarized in the last row of Table 10. Cross validation was not performed on ASSC-SVM models.

$\quad$ Because subsets of the testing cases were used in runs of this algorithm, the reference distribution of `functional` and `requires attention` wells will likely differ from the overall distribution for the entire set $\mathcal{P}$ (i.e. the no information rates will differ for different subsets of $\mathcal{P}$). While the Accuracy values are certainly informative, they are not as reliable as Kappa statistics to compare ASSC-SVM to other classification methods. Because the Kappa statistic takes into account the accuracy that would be generated simply by chance [@kuhn_johnson] it will be best served to compare ASSC-SVM models to each other as well as to all other models presented here.   
...
```{r, message=F, warning=F, fig.height=.5, echo = F, fig.align="center", fig.width = 7}
library(ggplot2)
ggplot() + ggtitle("Table 10", subtitle = "Actual and 10-Fold Cross Validation Accuracies and Kappa Statistics for Comparison of All Models Presented               ") + theme_minimal()
```
\begin{center}
\footnotesize
\begin{tabular}{l l c c c c}
    & & \multicolumn{2}{ c }{Actual} & \multicolumn{2}{c}{10-Fold CV} \\ \cline{3-6}
    & Model & Accuracy & Kappa & CV-Accuracy & CV-Kappa \\ \hline
    \multirow{ 2}{*}{Logistic Regression}& 13 Predictors & 0.6855 & 0.3703 & 0.7861 & 0.5148 \\ 
    & 15 Predictors & 0.6883 & 0.3748 & 0.7911 & 0.5251 \\ \hline
    \multirow{ 2}{*}{SVM}& 13 Predictors & 0.6754 & 0.3533 & 0.8147 & 0.5785 \\ 
    & 15 Predictors & 0.6759 & 0.3535 & 0.8192 & 0.5909 \\ \hline
    \multirow{ 2}{*}{Random Forest}& 13 Predictors & 0.6711 & 0.3455 & 0.8413 & 0.6352 \\ 
    & 15 Predictors & 0.6790 & 0.3568 & 0.8450 & 0.6464 \\ \hline
    \multirow{ 2}{*}{ASSC-SVM}& 13 Predictors & 0.6944 & 0.3904 & - & - \\ 
    & 15 Predictors & 0.7255 & 0.4486 & - & - \\ \hline
\end{tabular}
\end{center}
...
## STEP 0
 # create small workable data sets
 test_15_target <- cbind(test_15, test_y)
 test_15_small <- test_15_target[sample(nrow(test_15_target), 300),]
 train_15_small <- train_15
```  
...
```{r, echo = FALSE, eval = F}
 ...
 d_15 <- function(u, x){
   ...
   # continuous variables (2) <- euclidean distance
   # combine linearly and give a wieght to the categorical variable distance so that it has a reasonable amount of say in the distance between the two cases being compared
   dissimilarity <- sum((u[1:4]-x[1:4])^2) + tau^2*sum(u[5:15] != x[5:15])
 }
  
 # w is the "association matrix" higher values in this matrix indicate a higher association between the train and test cases being compared.
 w <- matrix(nrow = nrow(test_15_small), ncol = nrow(train_15_small))
 for(j in 1:nrow(test_15_small)){
   for(i in 1:nrow(train_15_small)){
     w[j,i] <- exp(-d_15(subset(test_15_small[j,], select = -c(test_y)), subset(train_15_small, select = -c(status_group))[i,])/lambda)
   }
 } 
```
...
```{r, echo = FALSE, eval = F}
 ### STEP 2
 # selection probability matrix
 p <- matrix(nrow = nrow(test_15_small), ncol = nrow(train_15_small))
 for(j in 1:nrow(test_15_small)){
   p[j,] <- w[j,]/apply(w, 1, sum)[j]
 } 
```
...
```{r, echo = FALSE, eval = F}
 ...
 B <- 201  # number of samples to take and vote 
 predictions <- matrix(nrow = nrow(test_15_small), ncol = B)
 ...
 for(b in 1:B){
   ...
   for(j in 1:nrow(test_15_small)){  
     ...
     for(l in 1:q){
       ...
       active_set_b <- rbind(active_set_b, train_15_small[index,])
     ...
     if(length(unique(active_set_b$status_group)) == 1){
       ...
       model_svm_b <- svm(status_group ~ ., data = active_set_b, kernel="radial", cost = 10)
       predictions[j, b] <- ifelse(predict(object = model_svm_b, newdata = subset(test_15_small[j,], select = -c(test_y))) == "functional", "functional", "requires attention")
     }
   }
 } 
...
```{r, echo = FALSE, eval = F}
 ...
 preds_assc <- ifelse(apply(predictions, 1, voting) >= (B+1)/2, "functional", "requires attention")
 confusionMatrix(preds_assc, test_15_small$test_y)
```


```{r, echo = FALSE, eval = F}
###################### RUN THIS CODE TONIGHT APRIL 13################
## STEP 0
 # create small workable data sets
 test_13_target <- cbind(test_13, test_y)
 test_13_small <- test_13_target[sample(nrow(test_13_target), 300),]
 ...
 ### STEP 1
 # dissimilarity/distance function
 tau <- 1.8  # set weight for categorical variable distance peice of function
 lambda <- 4
...
## Appendix  
__Logistic Regression Equations__  
__13 Predictor Variables__
\begin{singlespace}
\small
\vspace{-5mm}
$$
...
& + \beta_{11,1}I_{\{x_{11} = enough\}}+ \beta_{11,2}I_{\{x_{11} = insufficient\}}+ \beta_{11,3}I_{\{x_{11} = seasonal\}} + \beta_{11,4}I_{\{x_{11} = unkown\}} \\ 
&  + \beta{12,1}I_{\{x_{12} = surface\}}  + \beta{12,1}I_{\{x_{12} = unknown\}} \\
& + \beta_{13,1}I_{\{x_{13} = communal standpipe\}}+ \beta_{13,2}I_{\{x_{13} = communal standpipe multiple\}} + \beta_{13,3}I_{\{x_{13} = hand pipe\}} \\ &+ \beta_{13,4}I_{\{x_{13} = hand pipe\}} +  \beta_{13,5}I_{\{x_{13} = improved spring\}} + \beta_{13,1}I_{\{x_{13} = other\}} + \beta_{14,1}x_{14} + \beta_{15,1}x_{15}
\end{aligned}
$$
...
The largest advantages of implementing a logistic regression model are interpretability and computational convenience.
