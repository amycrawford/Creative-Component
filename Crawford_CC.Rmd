...
\begin{tabular}{l r c c}
    ...
    Prediction & \multicolumn{1}{r|}{functional} & `r cm_logistic_regression_13$table[1,1]` & `r cm_logistic_regression_13$table[1,2]` \\
    & \multicolumn{1}{r|}{requires attention} & `r cm_logistic_regression_13$table[2,1]` & `r cm_logistic_regression_13$table[2,2]` \\
\end{tabular}
\begin{tabular}{llrl}
    &&& \\
    &    &   Kappa :&`r round(cm_logistic_regression_13$overall[2], 4)`\\
    &    &   Accuracy :&`r round(cm_logistic_regression_13$overall[1], 4)`\\
    &    &   Sensitivity :&`r round(cm_logistic_regression_13$byClass[1], 4)` \\               
    &    &   Specificity :&`r round(cm_logistic_regression_13$byClass[2], 4)`\\
\end{tabular}
\end{center}
\vspace{1cm}
...
\begin{tabular}{l r c c}
    & & \multicolumn{2}{ c }{Reference} \\
    & & functional & requires attention \\ \cline{3-4} 
    Prediction & \multicolumn{1}{r|}{functional} & `r cm_logistic_regression_15$table[1,1]` & `r cm_logistic_regression_15$table[1,2]` \\
    & \multicolumn{1}{r|}{requires attention} & `r cm_logistic_regression_15$table[2,1]` & `r cm_logistic_regression_15$table[2,2]` \\
\end{tabular}
\begin{tabular}{llrl}
    &&& \\
    &    &   Kappa : &`r round(cm_logistic_regression_15$overall[2], 4)`\\
    &    &   Accuracy :&`r round(cm_logistic_regression_15$overall[1], 4)`\\
    &    &   Sensitivity :&`r round(cm_logistic_regression_15$byClass[1], 4)` \\               
    &    &   Specificity :&`r round(cm_logistic_regression_15$byClass[2], 4)`\\
\end{tabular}
\end{center}
\vspace{1cm}
...
$\quad$ Two SVM models were fit using the `svm` function with a _radial_ kernal in the `e1071` package in `R`. The cost paramater was tuned using 10-fold cross validation in the `caret` package. The model fit with 13 predictors was tuned to have a cost parameter of 50 and the model fit with 15 predictors was tuned to have a cost parameter of . The cost paramater was tuned using repeated 10-fold cross validation with the `tune` function in the `e1071` package in `R`. The confusion matrix and prediction statistics are included in Table 5. We see here that classification using a an SVM model yields slightly lower values of accuracy and the Kappa statistic, but still keeps a Kappa value above 0.3.
...
```{r, message=F, warning=F, fig.height=.5, echo = F, fig.align="center", fig.width = 7}
ggplot() + ggtitle("Table 5", subtitle = "Confusion Matrix and Statistics for Classifaction by Support Vector Machine (SVM) 13 predictors                                           ") + theme_minimal()
```
...
```{r, message=F, warning=F, fig.height=.5, echo = F, fig.align="center", fig.width = 7}
ggplot() + ggtitle("Table 6", subtitle = "Confusion Matrix and Statistics for Classifaction by Support Vector Machine (SVM) 15 predictors                                          ") + theme_minimal()
```
...
```{r, message=F, warning=F, fig.height=.5, echo = F, fig.align="center", fig.width = 7}
library(ggplot2)
ggplot() + ggtitle("Table ?", subtitle = "Compare Models                                           ") + theme_minimal()
```
\begin{center}
\footnotesize
\begin{tabular}{l l c c c c}
    ...
    \multirow{ 2}{*}{Logistic Regression}& 13 predictors & `r round(cm_logistic_regression_13$overall[1],4)` & `r round(cm_logistic_regression_13$overall[2],4)` & `r round(model_logistic_regression_13_CV$results[2],4)` & `r round(model_logistic_regression_13_CV$results[3],4)` \\ 
    & 15 predictors & `r round(cm_logistic_regression_15$overall[1],4)` & `r round(cm_logistic_regression_15$overall[2],4)` & `r round(model_logistic_regression_15_CV$results[2],4)` & `r round(model_logistic_regression_15_CV$results[3],4)` \\ \hline
    \multirow{ 2}{*}{SVM}& 13 predictors & 2 & 2 & 0.8192 & 0.5785 \\ 
    & 15 predictors & 2 & 2 & 0.8192 & 0.5909 \\ \hline
    \multirow{ 2}{*}{Random Forest}& 13 predictors & 2 & 2 & 3 & 3 \\ 
    & 15 predictors & 2 & 2 & 3 & 3 \\ \hline
    \multirow{ 2}{*}{ASSC-SVM}& 13 predictors & 2 & 2 & - & - \\ 
    & 15 predictors & 2 & 2 & - & - \\ \hline
\end{tabular}
\end{center}
